...
class JobAssociateCandidates extends Model
{
    ...
    public static function getAssociatedCandidatesByJobId($job_id){
        ...
        $query = $query->select('candidate_basicinfo.id as id', 'candidate_basicinfo.fname as fname', 'candidate_basicinfo.lname as lname',
            'candidate_basicinfo.email as email', 'users.name as owner','job_associate_candidates.shortlisted');
